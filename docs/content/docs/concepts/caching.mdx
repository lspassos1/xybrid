...
- expired-entry cleanup, targeted model deletion, and full clearing.
...
```dart
final status = Xybrid.modelCacheStatus();
for (final entry in Xybrid.modelCacheEntries()) {
  print('${entry.modelId}: ${entry.sizeBytes} bytes at ${entry.path}');
}

// Remove one model without disturbing the rest of the cache.
final removed = Xybrid.removeCachedModel('kokoro-82m');
```
