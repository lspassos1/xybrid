...
Live ASR accepts mono 16 kHz `Float32` PCM. `feedAsync` only queues samples;
...
input.installTap(onBus: 0, bufferSize: 320, format: format) { buffer, _ in
    ...
    Task { try await asr.feedAsync(samples: samples) }
}
engine.prepare()
try engine.start()
...
different hardware format, convert it to mono 16 kHz before calling `feedAsync`.
